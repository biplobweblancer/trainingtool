    ...
    public function dashboardTotalsuoeradmin(Request $request)
    {
        ...
        try {
            ...
            if (strtolower($userType->role->name) == "superadmin") {
                ...
                $data['running_batch'] = 0;
                $running_batch = TrainingBatchSchedule::whereHas('isStatus2')
                    ->orWhereHas('isStatus3')
                    ->count();

                $lastClass = BatchScheduleDetail::latest('id')->first();

                if (($lastClass->status != 3)) {
                    $data['running_batch'] = $running_batch;
                }
                ...
                $data['running_batch'] = 0;
                $running_batch = TrainingBatchSchedule::whereHas('trainingBatch', function ($query) use ($provider_id) {
                    $query->where('provider_id', $provider_id);
                })->whereHas('isStatus2')
                    ->orWhereHas('isStatus3')
                    ->count();
